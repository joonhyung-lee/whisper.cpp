...
./build/bin/ros2-whisper-stream -m ./models/ggml-tiny.bin -t 8 --step 500 --length 5000 -l ko
./build/bin/ros2-whisper-stream -m ./models/ggml-tiny.bin -t 8 --step 0 -l ko
*/
...
int main(int argc, char ** argv) {
    ...
    auto node = std::make_shared<rclcpp::Node>("whisper_ros2_stream_node");
    auto publisher = node->create_publisher<std_msgs::msg::String>("recognized_text", 10);
    RCLCPP_INFO(node->get_logger(), "Whisper ROS2 stream node started. Publishing to /recognized_text");
    ...
    while (is_running && rclcpp::ok()) {
        ...
        // run the inference
        {
            ...
            // Publish the combined text to ROS2 topic
            if (!combined_text.empty()) {
                auto message = std_msgs::msg::String();
                message.data = combined_text;
            ...
            fflush(stdout);
            // Spin ROS2 node to process callbacks (if any)
